...
const ProjectCard: React.FC<{ project: Project; onViewDetails: () => void }> = ({
  project,
  onViewDetails,
}) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex flex-col items-center hover:shadow-2xl transition-shadow duration-300 font-montserrat cursor-pointer group">
      ...
      <p className="text-xs text-gray-500 text-center mb-1">Starting at <span className="text-[#eb1478] font-bold">100 AED</span></p>
      <p className="text-xs text-gray-600 text-center mb-1">Raised: <span className="font-semibold">{project.raised.toLocaleString()} AED</span> / Goal: <span className="font-semibold">{project.goal.toLocaleString()} AED</span></p>
